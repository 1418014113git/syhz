...
public class TrainWorkorderService {
	...
	private static final int AUDIT_STATUS_4 = 4; // 0待审核
	...
	private WorkflowEntityBo verifyData(Map<String, String> headers, Map<String, Object> requestBody)
		...
		WorkflowEntityBo workflowEntityBo = WorkflowEntityBo.workflowMapToEntityBo(headers, requestBody);
		return workflowEntityBo;
	}

	private Object saveWorkorder(IBaseService baseService, WorkflowEntityBo workflowEntityBo) throws Exception {
		...
		if (workflowEntityBo.getDraft() == 1) {
			workorderMap.put("auditStatus", AUDIT_STATUS_0);
		}
		if (workflowEntityBo.getDraft() == 0) {
			workorderMap.put("auditStatus", AUDIT_STATUS_4);
		}
		...
		workorderMap.put("currentAuditNode", 0);
	 ...
	 * @throws Exception
	 */
	private void saveWorkorderFlow(IBaseService baseService, String workId, WorkflowEntityBo workflowEntityBo)
			throws Exception {
		// 本单位审批节点数据
		...
		if (provinceDeptBo != null) {
			...
			if (!"".equals(provinceDepCode)) {
				if (myDepCode.equals(provinceDepCode)) { // 如果本部门code 与 省级code码相同 则直接调用本部门
					...
					Object provinceWorkId = saveWorkorderFlowDataProvince(baseService, workId, workflowEntityBo,
							provinceDeptBo);
		...
		if (cityDeptBo != null) {
			...
			if (!"".equals(cityDepCode)) {
				if (myDepCode.equals(cityDepCode)) { // 如果本部门code 与 市级code码相同 则直接调用本部门
					...
					Object cityWorkId = saveWorkorderFlowDataCity(baseService, workId, parentWorkId, parentDepCode,
							workflowEntityBo, cityDeptBo);
	 ...
	 * @param workId
	 *            审核主体表id
	 * @param provinceWorkId
	 *            省级节点审核id
	 * @param provinceDepCode
	 *            省级部门code
	 * @param workflowEntityBo
	 * @param cityDeptBo
	 *            市级部门code
	 ...
	 * @throws Exception
	 */
	private Object saveWorkorderFlowDataCity(IBaseService baseService, Object workId, Object provinceWorkId,
			String provinceDepCode, WorkflowEntityBo workflowEntityBo, DeptEntityBo cityDeptBo) throws Exception {
		// 处理 保存市级审批节点数据
	 ...
	 * @throws Exception
	 */
	private Object saveWorkorderFlowDataMe(IBaseService baseService, Object workId, Object parentWorkId,
			Object parentDepCode, WorkflowEntityBo workflowEntityBo, DeptEntityBo myDeptBo, Object auditDeptNode)
			throws Exception {
		// 处理 保存市级审批节点数据
		Map<String, Object> workorderFlowMap = new HashMap<String, Object>();
		workorderFlowMap.put("workId", workId);
